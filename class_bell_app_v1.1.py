...
class BellDialog(QDialog):
    ...
    def get_bells(self):
        return self.start_bell_input.text().strip(), self.end_bell_input.text().strip()
...
class ClassBellApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.weather_timer_short.start(60 * 60 * 1000)
        ...
        self.action_start.triggered.connect(lambda: self.set_schedule_type(self.edit_table.selectedItems(), "수업시작"))
        self.addAction(self.action_start)

        self.action_end = QAction("종료 설정", self)
        self.action_end.setShortcut(Qt.Key_D)
        self.action_end.triggered.connect(lambda: self.set_schedule_type(self.edit_table.selectedItems(), "종료"))
        self.addAction(self.action_end)

        self.action_misc = QAction("기타시작 설정", self)
        self.action_misc.setShortcut(Qt.Key_E)
        self.action_misc.triggered.connect(lambda: self.set_schedule_type(self.edit_table.selectedItems(), "기타시작"))
        self.addAction(self.action_misc)

        self.action_insert = QAction("스케줄 삽입", self)
        self.action_insert.setShortcut(Qt.Key_I)
        self.action_insert.triggered.connect(self.add_below_schedule)
    ...
    def fetch_forecast_weather(self):
        ...
        retries = 3
        now = datetime.now()
        base_time_options = ["2300", "2000", "1700", "1400", "1100", "0800", "0500", "0200"]  # 가능한 예보 시간대
        for attempt in range(retries):
            try:
                ...
                response_fcst = requests.get(url_fcst, timeout=15)  # 타임아웃 15초로 증가
    ...
    def update_weather_data(self):
        if self.temp is not None and self.temp_min is not None and self.temp_max is not None and self.weather_desc is not None and self.icon is not None and self.city_name is not None:
            # 현재 온도가 최고/최저 온도를 초과하거나 낮으면 갱신
            if self.temp > self.temp_max:
                self.temp_max = self.temp
            if self.temp < self.temp_min:
                self.temp_min = self.temp
            self.weather_data = f"{self.icon} {self.city_name} 날씨: {self.weather_desc}, {self.temp:.1f}°C (최저 {self.temp_min:.1f}°C / 최고 {self.temp_max:.1f}°C, {datetime.now().strftime('%H:%M')} 갱신)"
        else:
            self.weather_data = "날씨 정보를 불러오는 중..."
    ...
    def setup_ui(self):
        ...
        <ul>
            ...
            <ul>
                ...
                <li><b>ESC 키</b>: 편집종료.</li>
                <li>우클릭 메뉴: "복사","붙여넣기,","삭제", "삽입", "수업시작/종료/기타시작" 선택.</li>
    ...
    def eventFilter(self, source, event):
        if source == self.edit_table and event.type() == QEvent.KeyPress:
            ...
            #print(f"Key pressed: {key}, Text: {event.text()}")
    ...
    def create_list(self):
        ...
        if ok and name and name not in self.schedule_lists:
            ...
            self.update_edit_table()
            self.save_config()
            self.status_label.setText(f"새 시간표 목록 '{name}'이 생성되었습니다.")
        elif name in self.schedule_lists:
            self.status_label.setText("이미 존재하는 이름입니다.")

    def rename_list(self):
        current_name = self.schedule_combo.currentText()
        if current_name == "기본 스케줄":
            self.status_label.setText("'기본 스케줄'은 이름을 변경할 수 없습니다.")
            return
        ...
        if ok and new_name and new_name not in self.schedule_lists:
            ...
            self.save_config()
            self.status_label.setText(f"시간표 목록 이름이 '{new_name}'으로 변경되었습니다.")
    ...
    def clone_list(self):
        ...
        if ok and new_name and new_name not in self.schedule_lists:
            ...
            self.update_edit_table()
            self.save_config()
            self.status_label.setText(f"'{current_name}'이 '{new_name}'으로 복제되었습니다.")
        elif new_name in self.schedule_lists:
            self.status_label.setText("이미 존재하는 이름입니다.")
    ...
    def delete_list(self):
        current_name = self.schedule_combo.currentText()
        if current_name == "기본 스케줄":
            self.status_label.setText("'기본 스케줄'은 삭제할 수 없습니다.")
            return
        ...
        if reply == QMessageBox.Yes:
            ...
            self.update_edit_table()
            self.save_config()
            self.status_label.setText(f"'{current_name}'이 삭제되었습니다.")

    def load_list(self, name):
        self.current_list = name
        self.update_edit_table()
        self.save_config()
        self.status_label.setText(f"'{name}' 시간표가 로드되었습니다.")
    ...
    def load_schedule(self):
        ...
        try:
            ...
            self.update_edit_table()
            self.update_today_schedule()
            self.status_label.setText(f"시간표가 {file_name}에서 '{new_name}'으로 불러와졌습니다.")
    ...
    def check_time(self):
        ...
        for item in self.schedule_lists[self.current_list][current_day]:
            ...
            if item["active"] == "예" and schedule_time == current_time_str and self.last_played_time != schedule_time:
                ...
                if "수업시작" in item["desc"] or "교시 시작" in item["desc"] or "기타시작" in item["desc"]:
                    self.player.setMedia(QMediaContent(QUrl.fromLocalFile(resource_path(self.start_bell))))
                    self.player.play()
                    logger.info(f"Playing start bell at {current_time_str}")
                    bell_triggered = True
                elif "종료" in item["desc"] or "교시 종료" in item["desc"]:
                    self.player.setMedia(QMediaContent(QUrl.fromLocalFile(resource_path(self.end_bell))))
                    self.player.play()
                    logger.info(f"Playing end bell at {current_time_str}")
                    bell_triggered = True
    ...
    def copy_schedule(self):
        ...
        selected_items = self.edit_table.selectedItems()
        if not selected_items:
            self.status_label.setText("복사할 셀이 선택되지 않았습니다.")
            return
        self.copied_data = []
        for item in selected_items:
            row = item.row()
        ...
        if self.copied_data:
            self.status_label.setText(f"{len(self.copied_data)}개의 일정이 복사되었습니다.")
        else:
            self.status_label.setText("복사할 유효한 일정이 없습니다.")

    def paste_schedule(self):
        # 복사된 일정을 선택된 위치에 붙여넣기
        if not self.copied_data:
            self.status_label.setText("붙여넣을 데이터가 없습니다.")
            return
        selected_items = self.edit_table.selectedItems()
        if not selected_items:
            self.status_label.setText("붙여넣을 위치가 선택되지 않았습니다.")
            return
        ...
        # 테이블 갱신
        self.update_edit_table()
        self.status_label.setText(f"{len(self.copied_data)}개의 일정이 붙여넣어졌습니다.")
