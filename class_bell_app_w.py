        ...
        try:
            ...
            if os.path.exists(cache_file):
                ...
                if datetime.now() - last_updated < cache_duration:
                    ...
                    print(f"Using cached data: {self.weather_data}")
                    return
            ...
            nx, ny = city_info["nx"], city_info["ny"]

            # 기상청 단기예보 API 호출
            now = datetime.now()
            base_date = now.strftime("%Y%m%d")
            base_time = ((now.hour // 3) * 3)  # 3시간 간격 (02, 05, 08, 11, 14, 17, 20, 23)
            base_time_str = f"{base_time:02d}00"

            api_key = "your_kma_api_key"  # 발급받은 인증키로 교체
            url = (
                f"http://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getVilageFcst"
                f"?serviceKey={api_key}&numOfRows=290&pageNo=1"
                f"&base_date={base_date}&base_time={base_time_str}&nx={nx}&ny={ny}&dataType=JSON"
            )
            ...
            for item in items:
                ...
                if fcst_date == base_date and int(fcst_time[:2]) >= now.hour:
                    if item["category"] == "TMP":
                        temp = float(item["fcstValue"]) if temp is None else temp
                        forecast_temps.append(float(item["fcstValue"]))
                    elif item["category"] == "SKY" and sky is None:
                        sky = item["fcstValue"]
                    elif item["category"] == "PTY" and pty is None:
                        pty = item["fcstValue"]

            # 하루 예보에서 최저/최고 온도 계산
            temp_min = min(forecast_temps[:8])  # 다음 24시간 (3시간 간격 8개)
            temp_max = max(forecast_temps[:8])

            # 날씨 상태 매핑
            pty_map = {"0": "", "1": "비", "2": "비/눈", "3": "눈", "4": "소나기"}
            sky_map = {"1": "맑음", "3": "구름많음", "4": "흐림"}
            weather_desc = pty_map.get(pty, sky_map.get(sky, "알 수 없음")) if pty != "0" else sky_map.get(sky, "알 수 없음")

            icon = (
                "☀️" if "맑음" in weather_desc else
                "☁️" if "흐림" in weather_desc or "구름" in weather_desc else
                "🌧️" if "비" in weather_desc or "눈" in weather_desc or "소나기" in weather_desc else
                "⚠️"
            )
            ...
            print(f"Updated cache with: {self.weather_data}")
