...
class ClassBellApp(QMainWindow):
    ...
    def setup_ui(self):
        ...
        info_text = (
            ...
            "<b>* 노엠스쿨:</b> <a href='https://noemschool.org'>https://noemschool.org</a> ☎ 0507-1301-6215 (신입생 상시 모집)<br>"
        ...
        <h3>1. 프로그램 시작하기</h3>
        <ul>
            <li><b>실행</b>: 바탕화면 아이콘을 더블클릭하거나, 컴퓨터를 켜면 자동으로 시작돼요 <br>(설정 필요 시 아래 참고).</li>
        ...
        <ol>
            ...
            <li><b>자동 실행</b>: "윈도우 부팅시 자동 시작" 체크는 관리자 모드로 실행해야 설정돼요 <br>(우클릭 → "관리자 권한으로 실행"). 일반 모드에서는 체크가 안 돼요.</li>
            <li><b>최소화</b>: 창을 최소화하거나 끄면 오른쪽 아래 트레이에서 🔔 아이콘을 더블클릭하여 다시 열 수 있어요.</li>
        </ol>
        ...
        <p>이 프로그램은 노엠스쿨에서 선생님들과 학생들들을 위해 만든 무료 프로그램이에요. <br>편리하게 사용해 주세요! 🌈<br>
    ...
    def eventFilter(self, source, event):
        if source == self.edit_table and event.type() == QEvent.KeyPress:
            ...
            elif key == Qt.Key_E and current_items:
                self.set_schedule_type(current_items, "기타시작")
                self.update_edit_table()
                self.status_label.setText("E 키: 기타시작 설정")
                return True

            elif key == Qt.Key_I and current_items:
                self.add_below_schedule()
                return True
    ...
    def check_time(self):
        ...
        for item in self.schedule_lists[self.current_list][current_day]:
            ...
            if item["active"] == "예" and schedule_time == current_time_str and self.last_played_time != current_time:
