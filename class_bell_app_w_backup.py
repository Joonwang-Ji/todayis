...
logger.setLevel(logging.INFO)  # 디버깅 이전에는 INFO 수준이었을 가능성
...
def resource_path(relative_path):
    ...
    if not os.path.exists(path):
        logging.error(f"Resource not found: {path}")
    return path
...
class ClassBellApp(QMainWindow):
    def __init__(self):
        ...
        if not self.shared_mem.create(1):
            QMessageBox.information(None, "오늘은 v1.0", "프로그램이 이미 실행 중입니다.")
            sys.exit(0)

        super().__init__()
        self.setWindowTitle("오늘은 v1.0")
        ...
        self.start_bell = "start_bell.wav"
        self.end_bell = "end_bell.wav"
        ...
        self.tray_icon.setToolTip("오늘은 v1.0")
        tray_menu = QMenu()
        show_action = QAction("보이기", self)
        show_action.triggered.connect(self.show)
        tray_menu.addAction(show_action)
        exit_action = QAction("종료하기", self)
        ...
        # 날씨 관련 변수 추가
        self.weather_api_key = "9f391ab2797cc7de5a5ec117943c0cc8"  # 나중에 실제 키로 교체
        self.weather_city = "Seoul"  # 기본 도시 (서울)
        self.weather_data = "날씨 정보를 불러오는 중..."  # 초기값

        # 날씨 업데이트 타이머 추가
        self.weather_timer = QTimer(self)
        self.weather_timer.timeout.connect(self.update_weather)
        self.weather_timer.start(600000)  # 10분(600,000ms)마다 업데이트
        ...
        self.update_weather()  # 최초 실행 시 즉시 날씨 업데이트
        self.show()
    ...
    def fetch_weather(self):
        try:
            ...
            # print(f"IP 위치: {ip_data.get('city', 'Unknown')} / {ip_data.get('region', 'Unknown')}") # 위치 정밀도 테스트
            ...
            city_map = {
                ...
                "Suwon": "수원시", "Yongin": "용인시", "Seongnam": "성남시", "Bucheon": "부천시",
                "Hwaseong": "화성시", "Ansan": "안산시", "Anyang": "안양시", "Pyeongtaek": "평택시",
                "Siheung": "시흥시", "Gimpo": "김포시", "Gwangju-si": "광주시", "Goyang": "고양시",
                "Namyangju": "남양주시", "Paju": "파주시", "Uijeongbu": "의정부시", "Yangju": "양주시",
                "Icheon": "이천시", "Osan": "오산시", "Anseong": "안성시", "Gunpo": "군포시",
                "Uiwang": "의왕시", "Hanam": "하남시", "Yeoju": "여주시", "Dongducheon": "동두천시",
                "Gwacheon": "과천시", "Guri": "구리시", "Pocheon": "포천시", "Chuncheon": "춘천시",
                "Wonju": "원주시", "Gangneung": "강릉시", "Donghae": "동해시", "Taebaek": "태백시",
                "Sokcho": "속초시", "Samcheok": "삼척시", "Cheongju": "청주시", "Chungju": "충주시",
                "Jecheon": "제천시", "Cheonan": "천안시", "Gongju": "공주시", "Boryeong": "보령시",
                "Asan": "아산시", "Seosan": "서산시", "Nonsan": "논산시", "Gyeryong": "계룡시",
                "Dangjin": "당진시", "Jeonju": "전주시", "Gunsan": "군산시", "Iksan": "익산시",
                "Jeongeup": "정읍시", "Namwon": "남원시", "Gimje": "김제시", "Mokpo": "목포시",
                "Yeosu": "여수시", "Suncheon": "순천시", "Naju": "나주시", "Gwangyang": "광양시",
                "Pohang": "포항시", "Gyeongju": "경주시", "Gimcheon": "김천시", "Andong": "안동시",
                "Gumi": "구미시", "Yeongju": "영주시", "Yeongcheon": "영천시", "Sangju": "상주시",
                "Mungyeong": "문경시", "Gyeongsan": "경산시", "Changwon": "창원시", "Jinju": "진주시",
                "Tongyeong": "통영시", "Sacheon": "사천시", "Gimhae": "김해시", "Miryang": "밀양시",
                "Geoje": "거제시", "Yangsan": "양산시", "Jeju": "제주시", "Seogwipo": "서귀포시"
            }
            city_name = city_map.get(city_from_ip, city_from_ip)

            # 현재 날씨 (영어 원문과 한국어 번역 모두 가져오기)
            url_current_en = f"http://api.openweathermap.org/data/2.5/weather?q={city_from_ip}&appid={self.weather_api_key}&units=metric"
            url_current_kr = f"http://api.openweathermap.org/data/2.5/weather?q={city_from_ip}&appid={self.weather_api_key}&units=metric&lang=kr"
            response_current_en = requests.get(url_current_en, timeout=5)
            response_current_kr = requests.get(url_current_kr, timeout=5)
            response_current_en.raise_for_status()
            response_current_kr.raise_for_status()
            data_current_en = response_current_en.json()
            data_current_kr = response_current_kr.json()

            weather_desc_en = data_current_en["weather"][0]["description"]  # 영어 원문
            weather_desc_kr = data_current_kr["weather"][0]["description"]  # 한국어 번역
            # print(f"원문 (영어): {weather_desc_en}, 번역 (한국어): {weather_desc_kr}")
            temp = data_current_kr["main"]["temp"]

            # 하루 예보로 최저/최고 온도
            url_forecast = f"http://api.openweathermap.org/data/2.5/forecast?q={city_from_ip}&appid={self.weather_api_key}&units=metric&lang=kr"
            response_forecast = requests.get(url_forecast, timeout=5)
            response_forecast.raise_for_status()
            data_forecast = response_forecast.json()

            temps = [entry["main"]["temp"] for entry in data_forecast["list"][:8]]  # 24시간 (3시간 간격 8개)
            temp_min = min(temps)
            temp_max = max(temps)

            # OpenWeatherMap 모든 날씨 상태 매핑 (영어 원문 기준)
            weather_map = {
                "thunderstorm with light rain": "천둥번개와 약한 비",
                "thunderstorm with rain": "천둥번개와 비",
                "thunderstorm with heavy rain": "천둥번개와 큰 비",
                "light thunderstorm": "약한 천둥번개",
                "thunderstorm": "천둥번개",
                "heavy thunderstorm": "강한 천둥번개",
                "ragged thunderstorm": "불규칙한 천둥번개",
                "thunderstorm with light drizzle": "천둥번개와 약한 이슬비",
            ...
            simple_desc = weather_map.get(weather_desc_en, weather_desc_kr)  # 영어 원문 기준, 오류 시 한국어 사용
            icon = (
                "☀️" if "맑음" in simple_desc else
                "☁️" if "흐림" in simple_desc or "구름" in simple_desc else
                "🌫️" if "안개" in simple_desc else
                "🌧️" if "비" in simple_desc or "눈" in simple_desc or "소나기" in simple_desc else
                "⚡" if "천둥번개" in simple_desc else
                "🌪️" if "토네이도" in simple_desc or "돌풍" in simple_desc else
                "❄️" if "진눈깨비" in simple_desc else
                "⚠️"  # 기본값
            )
            self.weather_data = f"{icon} {city_name} 날씨: {simple_desc}, {temp:.1f}°C (최저 {temp_min:.1f}°C / 최고 {temp_max:.1f}°C)"
        except requests.exceptions.RequestException as e:
            self.weather_data = "⚠️ 날씨 정보를 가져올 수 없습니다."
            logging.error(f"Failed to fetch weather: {str(e)}")
        except KeyError as e:
            self.weather_data = "⚠️ 날씨 데이터 형식이 잘못되었습니다."
...
if __name__ == "__main__":
    logging.info("Starting application")  # 현재 로그 출력 지점
    try:
        app = QApplication(sys.argv)
        logging.debug("QApplication initialized")
        window = ClassBellApp()
        logging.debug("ClassBellApp instance created")
        sys.exit(app.exec_())
    except Exception as e:
