...
class ClassBellApp(QMainWindow):
    ...
    def __init__(self):
        ...
        if not self.shared_mem.create(1):
            QMessageBox.information(None, "오늘은", "프로그램이 이미 실행 중입니다.")
            sys.exit(0)

        super().__init__()
        self.current_version = "1.2"
        self.setWindowTitle("오늘은 v{self.current_version}")
        ...
        self.version_timer.timeout.connect(self.check_version_update)
        self.version_timer.start(24 * 60 * 60 * 1000)
        self.check_version_update()

        self.downloading = False  # 다운로드 중 플래그
        ...
        self.tray_icon.setToolTip("오늘은 v{self.current_version}")
        tray_menu = QMenu()
    ...
    def check_version_update(self):
        try:
            version_url = "https://raw.githubusercontent.com/Joonwang-Ji/todayis/main/version.txt"
            response = requests.get(version_url, timeout=5)
            response.raise_for_status()
            latest_version = response.text.strip()

            if version.parse(latest_version) > version.parse(self.current_version):
                self.notify_version_update(latest_version)
        except requests.RequestException as e:
            logger.error(f"버전 확인 실패: {str(e)}")

    def notify_version_update(self, latest_version):
        if self.downloading:  # 이미 다운로드 중이면 알림 생략
            return

        msg = QMessageBox(self)
        msg.setWindowTitle("업데이트 알림")
        msg.setText(f"새 버전 v{latest_version}이(가) 출시되었습니다!\n현재 버전: v{self.current_version}\n자동으로 다운로드하고 설치하시겠습니까?")
        msg.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
        reply = msg.exec_()

        if reply == QMessageBox.Yes:
            self.start_download(latest_version)
        else:
            self.tray_icon.showMessage(
                "업데이트 알림",
                f"새 버전 v{latest_version}이(가) 출시되었습니다!",
                QSystemTrayIcon.Information,
                5000
            )

    def start_download(self, latest_version):
        self.downloading = True
        download_url = f"https://github.com/Joonwang-Ji/todayis/releases/download/v{latest_version}/ClassBellApp-v{latest_version}.exe"
        download_path = os.path.join(os.getenv('TEMP'), f"ClassBellApp-v{latest_version}.exe")

        # 다운로드 진행 상황 알림
        self.set_status_message("업데이트를 다운로드 중입니다...")
                ...
                with open(download_path, 'wb') as f:
                    for chunk in response.iter_content(chunk_size=8192):
                        if chunk:
                            ...
                            self.set_status_message(f"다운로드 중: {progress:.1f}%")

                # 다운로드 완료
                self.set_status_message("다운로드 완료. 설치를 시작합니다...")
                logger.info(f"Downloaded update to {download_path}")

                # 설치 실행 및 프로그램 종료
                subprocess.Popen([download_path])  # 설치 프로그램 실행
                QApplication.quit()  # 현재 프로그램 종료

            except requests.RequestException as e:
                logger.error(f"다운로드 실패: {str(e)}")
                self.set_status_message("업데이트 다운로드에 실패했습니다.")
                QMessageBox.critical(self, "오류", f"다운로드 실패: {str(e)}")
            except Exception as e:
                logger.error(f"설치 중 오류: {str(e)}")
                self.set_status_message("설치 중 오류가 발생했습니다.")
                QMessageBox.critical(self, "오류", f"설치 오류: {str(e)}")
            finally:
